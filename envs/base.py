"""Base environment class for CALM."""
...
class Env(object):
    """Base environment class for task-based agent evaluation."""
    ...
    def reset(self, task_index: Optional[int] = None) -> EnvResetResponse:
        ...
        self.actions = []
        initial_observation = self.user.reset(instruction=self.task.instruction if self.task else None)
        return EnvResetResponse(
            observation=initial_observation, info=EnvInfo(task=self.task, source="user")
        )

    def step(self, action: Action) -> EnvResponse:
        """Execute an action and return the environment response."""
    ...
    def get_data_hash(self) -> str:
        """Get a hash of the current data state."""
    ...
    def calculate_reward(self) -> RewardResult:
        ...
        if self.task and len(self.task.outputs) > 0:
            # Check outputs
            r_outputs = 1.0
            outputs = {}
