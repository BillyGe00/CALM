...
# 修改：使用你的 envs 而不是 tau_bench
from envs import get_env
from envs.types import SolveResult, RESPOND_ACTION_NAME, Action
...
class TauGreenAgentExecutor(AgentExecutor):
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        metrics["reward"] = res.reward  # 记录实际 reward 值
        # 使用阈值比较，避免浮点数精度问题
        result_bool = metrics["success"] = res.reward >= 0.99
        result_emoji = "✅" if result_bool else "❌"

        print("Green agent: Evaluation complete.")
        reward_percent = res.reward * 100
        print(f"Reward: {reward_percent:.1f}%, Success: {result_bool}")
        await event_queue.enqueue_event(
            new_agent_text_message(
                f"Finished. White agent success: {result_emoji}\n"
                f"Reward: {reward_percent:.1f}%\n"
